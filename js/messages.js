 ...
 * =========================================================================
 *
 * These messages enable Lightning-style game routing through a Casino hub.
 * The hub forwards commit-reveal secrets between player and house channels.
 *
 *   Hub → House:  HOUSE_REQUEST     (incoming game, be the house)
 *   House → Hub:  HOUSE_OFFER       (house's commit)
 *   Hub → House:  PLAYER_ACCEPTED   (player's commit + pick + bet)
 *   House → Hub:  HOUSE_REVEAL      (house's secret)
 *
 * The hub also handles standard GAME_OFFER, GAME_ACCEPT, GAME_REVEAL,
 * GAME_RESULT, GAME_BET_SIGNED, GAME_RESULT_SIGNED by forwarding between
 * the two channels. Those use the existing message constructors above.
 *
 *   Hub → House:  POOL_JOIN         (player wants to join the network)
 *   Hub → All:    POOL_STATUS       (network capacity broadcast)
 * ========================================================================= */

/**
 * HOUSE_REQUEST — Hub asks a player to be the house for a routed game.
 *
 * Sent from the Casino hub to a selected house player. The house
 * generates a secret, commits, and responds with HOUSE_OFFER.
 *
 * @param hashid    — House's channel hashid (with the hub)
 * @param gametype  — "flip", "dice", or "roulette"
 * @param range     — Number of outcomes (2, 6, or 36)
...
/**
 * HOUSE_OFFER — House responds with their commit.
 *
 * Sent from the house back to the hub. The hub forwards
 * the commit to the player as a standard GAME_OFFER.
 *
 * @param hashid      — House's channel hashid
...
/**
 * PLAYER_ACCEPTED — Hub forwards the player's commit to the house.
 *
 * After the player sends GAME_ACCEPT to the hub, the hub forwards
 * the player's commit, pick, and bet to the house via this message.
 * The house then signs the pessimistic balance.
 *
 * @param hashid        — House's channel hashid
 * @param playercommit  — SHA3(player_secret), forwarded from the real player
 * @param pick          — Player's chosen number (mirrored by hub)
 * @param betamt        — Bet amount
 * @param gametype      — Game type
 */
...
/**
 * HOUSE_REVEAL — House reveals their secret to the hub.
 *
 * Sent from the house after signing the pessimistic balance.
 * The hub forwards this to the player as a standard GAME_REVEAL.
 *
 * @param hashid      — House's channel hashid
 * @param housesecret — The house's actual secret (preimage of housecommit)
 */
...
/**
 * POOL_STATUS — Network capacity broadcast.
 *
 * Sent periodically from the hub to all connected players.
 * Contains aggregate network stats for UI display.
 *
 * @param online       — Number of online house players
 * @param totalCapacity — Total available house balance
 * @param maxFlip      — Max coin flip bet
 * @param maxDice      — Max dice bet
 * @param maxRoulette  — Max roulette bet
 */
